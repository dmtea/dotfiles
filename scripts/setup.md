...
| 01 | **System Base**  | apt packages + build-essential + user scripts → `~/.local/bin` + system XKB (`unipunct` layout) |
...
`.gitconfig` is generated by `setup-git.sh` (not a stow package). Keyboard layout is deployed at system level in module 01, not via stow.
